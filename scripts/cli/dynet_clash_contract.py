...
def command_hints(args: argparse.Namespace, primary_domains: list[dict[str, Any]]) -> dict[str, str]:
    ...
    return {
        "profile": (
            "uv --project scripts run python -m scripts.cli.clash_verge_profile "
            "--output-json .task/resources/clash-verge-access-profile.json "
            "--output-md .task/resources/clash-verge-access-profile.md"
        ),
        "manifest": (
            "uv --project scripts run python -m scripts.cli.real_access_blackbox plan "
        ...
        "clashRun": (
            "uv --project scripts run python -m scripts.cli.real_access_blackbox run "
            f"--manifest {manifest} --environment local-clash --label github-proof-clash"
        ),
        "dynetRun": (
            "uv --project scripts run python -m scripts.cli.dynet_probe_manifest "
