...
from dynet_trace.probe_batch import (
    DEFAULT_PROBE_BATCH_JSON,
    DEFAULT_PROBE_BATCH_MD,
    build_probe_batch,
    write_probe_batch_report,
)
...
def probe_batch_paths(
    ...
    paths.extend(Path(path) for path in args.attribution or [])
    if not paths:
        raise SystemExit("probe-batch requires at least one --attribution or a --manifest")
    return paths, manifest, manifest_path
...
def command_probe_batch(args: argparse.Namespace) -> int:
    paths, manifest, manifest_path = probe_batch_paths(args)
    ...
    write_json(output_json, batch)
    write_probe_batch_report(output_md, batch)
    failed_gates = failed_gate_names(batch)
    print(
        json.dumps(
            {
                ...
                "outputMd": str(output_md),
                "runs": batch["totals"]["runs"],
                "selectedBehind": batch["totals"]["selectedBehind"],
                "failedGates": failed_gates,
            },
            sort_keys=True,
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    probe_batch_parser = subparsers.add_parser(
        "probe-batch",
        help="aggregate saved probe-manifest attribution JSON reports",
    )
    probe_batch_parser.add_argument("--attribution", action="append")
    probe_batch_parser.add_argument(
        "--manifest",
        help="JSON manifest with summaries, thresholds, and optional outputs",
    )
    probe_batch_parser.add_argument("--output-json")
    probe_batch_parser.add_argument("--output-md")
    probe_batch_parser.add_argument("--min-repeat-runs", type=int)
    probe_batch_parser.add_argument(
        "--fail-on-gate",
        action="store_true",
        help="return non-zero when any batch gate fails",
    )
    probe_batch_parser.set_defaults(handler=command_probe_batch)
