...
def build_probe_attribution(summary_path: Path) -> dict[str, Any]:
    ...
    for row in evidence:
        ...
        row["suspectComponent"] = suspect_component(row)
    quality = candidate_quality_totals(evidence)
    return {
        "schema": PROBE_ATTRIBUTION_SCHEMA,
        "inputSummary": str(summary_path),
        "replay": summary.get("replay", {}),
        "totals": totals(evidence, quality),
        ...
        "bySuspectComponent": top(Counter(
            row["suspectComponent"]
            for row in evidence
            if row.get("suspectComponent")
        )),
...
def item_evidence(item: dict[str, Any], summary_path: Path) -> dict[str, Any]:
    ...
    rule = first_event(events, "rule-matched")
    decision = route or rule
    ...
    missing = missing_evidence(decision, graph, attempts, stages, item)
    route_fields = event_fields(decision or {})
    graph_fields = event_fields(graph or {})
    dialers = dialer_attempts(events)
    return {
        "id": item.get("id"),
        ...
        "dialerAttempts": dialers,
        "failureScope": item.get("failureScope") or failure_scope(dialers),
...
def candidate_sets(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for event in event_list(events, "outbound-candidate-set"):
        ...
        details = json_list_field(fields.get("candidatesJson"))
        rows.append({
            "scope": fields.get("scope") or "plan-candidate",
            ...
            "quality": candidate_quality_summary(details, fields.get("selected")),
        })
    return rows
...
def candidate_quality_summary(
    ...
    rows = quality_rows(details)
    if not rows:
        return {}
    best_score = max(row["score"] for row in rows)
    selected_rows = [row for row in rows if row["candidate"] == selected]
    selected_row = selected_rows[0] if selected_rows else None
    selected_score = selected_row["score"] if selected_row else None
    return {
        "bestScore": best_score,
        "bestCandidates": [
            row["candidate"] for row in rows if row["score"] == best_score
        ],
        "selectedScore": selected_score,
        "selectedReason": selected_row.get("reason") if selected_row else None,
        "selectedStale": selected_row.get("stale") if selected_row else None,
...
def quality_rows(details: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows = []
    for detail in details:
        ...
        if score is None:
            continue
        rows.append({
            "candidate": str(detail.get("to") or ""),
            "score": score,
...
def stage_evidence(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [stage_row(event) for event in event_list(events, "outbound-stage-finished")]
...
def missing_evidence(
    ...
    missing = []
    if decision is None:
        missing.append("route-or-rule-matched")
    if graph is None:
...
def classify(row: dict[str, Any], guardrails_clean: bool) -> str:
    ...
    if row["planDecisionCount"] and row["candidateSets"] and not row["graphSelected"]:
        return "plan-suspect"
    if candidate_quality_prefers_other(row["candidateSets"]):
        return "plan-suspect"
...
def candidate_quality_prefers_other(candidate_sets: list[dict[str, Any]]) -> bool:
    for candidate_set in candidate_sets:
        quality = candidate_set.get("quality")
        if not isinstance(quality, dict):
            continue
        if quality.get("selectedStale") is True:
            continue
        gap = int_value(quality.get("scoreGap"))
...
def candidate_quality_totals(rows: list[dict[str, Any]]) -> dict[str, Any]:
    sets = [
        (row, candidate_set)
        for row in rows
        for candidate_set in row.get("candidateSets", [])
        if isinstance(candidate_set.get("quality"), dict)
    ]
    gaps = [
        candidate_quality_gap_row(row, candidate_set)
        for row, candidate_set in sets
        if candidate_quality_gap(candidate_set) > 0
    ]
    return {
        "candidateSets": sum(len(row.get("candidateSets", [])) for row in rows),
        "withQuality": len(sets),
        "selectedBest": sum(1 for _, item in sets if candidate_quality_gap(item) == 0),
        "selectedBehind": len(gaps),
        "selectedStale": sum(
            1
            for _, item in sets
            if item.get("quality", {}).get("selectedStale") is True
        ),
        "maxScoreGap": max([item["scoreGap"] for item in gaps], default=0),
        "bySelectedReason": top(Counter(
            str(item.get("quality", {}).get("selectedReason") or "unknown")
            for _, item in sets
        )),
        "gaps": gaps,
    }
...
def candidate_quality_gap(candidate_set: dict[str, Any]) -> int:
    quality = candidate_set.get("quality")
    if not isinstance(quality, dict):
        return 0
    if quality.get("selectedStale") is True:
        return 0
    return int_value(quality.get("scoreGap")) or 0


def candidate_quality_gap_row(
    row: dict[str, Any],
    ...
    return {
        ...
        "scope": candidate_set.get("scope") or "plan-candidate",
        "plan": candidate_set.get("plan"),
        "selected": candidate_set.get("selected"),
        "selectedScore": quality.get("selectedScore"),
        "bestScore": quality.get("bestScore"),
        "scoreGap": int_value(quality.get("scoreGap")) or 0,
        "bestCandidates": quality.get("bestCandidates", []),
    }


def totals(rows: list[dict[str, Any]], quality: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "candidateQualityGaps": quality.get("selectedBehind", 0),
    }


def evidence_completeness(rows: list[dict[str, Any]]) -> dict[str, Any]:
    required = [
        "route-or-rule-matched",
...
def write_probe_attribution_report(path: Path, report: dict[str, Any]) -> None:
    ...
    if quality:
        ...
        lines.append(
            f"- Sets: `{quality['withQuality']}/{quality['candidateSets']}` with quality"
        )
        lines.append(
            f"- Selected best: `{quality['selectedBest']}` "
            f"behind=`{quality['selectedBehind']}` stale=`{quality['selectedStale']}` "
            f"maxGap=`{quality['maxScoreGap']}`"
        )
        for item in quality.get("bySelectedReason", []):
            lines.append(f"- reason `{item['key']}`: {item['count']}")
        append_candidate_quality_gaps(lines, quality)
    ...
    if report["failures"]:
        ...
        for item in report["failures"]:
            lines.append(
                ...
                f"class=`{item['classification']}` suspect=`{item.get('suspectComponent')}`"
            )
    path.write_text("\n".join(lines) + "\n")


def append_candidate_quality_gaps(
    lines: list[str],
    ...
    for item in gaps[:20]:
        lines.append(
            f"- `{item['id']}` {item['domain']} scope=`{item.get('scope', 'plan-candidate')}` "
            f"plan=`{item['plan']}` "
            f"selected=`{item['selected']}` gap=`{item['scoreGap']}` "
            f"best={','.join(item.get('bestCandidates', []))}"
        )
