...
def runtime_dns_forward_source(path: Path) -> dict[str, Any]:
    ...
    source["clean"] = runtime_dns_forward_clean(source)
    return source


def runtime_dns_forward_summary(sources: list[dict[str, Any]]) -> dict[str, Any]:
    return {
...
def runtime_dns_forward_clean(source: dict[str, Any]) -> bool:
    return (
        source["schema"] == DNS_FORWARD_SCHEMA
        ...
        and source["diagnosticQueries"] > 0
        and source["proxyForwardQueries"] > 0
        and source["terminalFailureQueries"] > 0
