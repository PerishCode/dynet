...
def runtime_failure_attribution_source(path: Path) -> dict[str, Any]:
    ...
    source = {
        ...
        **{field: int(totals.get(field) or 0) for field in COUNT_FIELDS},
        ...
        "profiles": count_keys(totals.get("profiles")),
        "missingEvidence": count_keys(totals.get("missingEvidence")),
        "privacy": empty_privacy_flags(),
    }
    source["clean"] = runtime_failure_attribution_clean(source)
    return source


def runtime_failure_attribution_summary(sources: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        **{field: sum(source[field] for source in sources) for field in COUNT_FIELDS},
        ...
        "profiles": merge_items(sources, "profiles"),
        "missingEvidence": merge_items(sources, "missingEvidence"),
        "sources": sources,
    }


def runtime_failure_attribution_clean(source: dict[str, Any]) -> bool:
    return (
...
def blocker_fields() -> list[str]:
    return "unknownSignals missingEvidenceSignals unknown".split()
