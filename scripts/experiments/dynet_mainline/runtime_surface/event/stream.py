...
def runtime_event_stream_source(path: Path) -> dict[str, Any]:
    ...
    source = {
        ...
        **{field: int(totals.get(field) or 0) for field in COUNT_FIELDS},
        ...
        "eventKindCounts": count_keys(totals.get("eventKindCounts")),
        "missingFieldNames": count_keys(totals.get("missingFieldNames")),
        "counterMismatchNames": count_keys(totals.get("counterMismatchNames")),
        "byteCounterMismatchNames": count_keys(totals.get("byteCounterMismatchNames")),
        "privacy": empty_privacy_flags(),
    }
    source["clean"] = runtime_event_stream_clean(source)
    return source


def runtime_event_stream_summary(sources: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        **{field: sum(source[field] for source in sources) for field in COUNT_FIELDS},
        ...
        "eventKindCounts": merge_items(sources, "eventKindCounts"),
        "missingFieldNames": merge_items(sources, "missingFieldNames"),
        "counterMismatchNames": merge_items(sources, "counterMismatchNames"),
        "byteCounterMismatchNames": merge_items(sources, "byteCounterMismatchNames"),
        "sources": sources,
    }


def runtime_event_stream_clean(source: dict[str, Any]) -> bool:
    return (
        source["schema"] == EVENT_STREAM_SCHEMA
