...
def runtime_outbound_retry_source(path: Path) -> dict[str, Any]:
    ...
    source["clean"] = runtime_outbound_retry_clean(source)
    return source


def runtime_outbound_retry_summary(sources: list[dict[str, Any]]) -> dict[str, Any]:
    return {
...
def runtime_outbound_retry_clean(source: dict[str, Any]) -> bool:
    return (
        source["schema"] == OUTBOUND_RETRY_SCHEMA
...
def blocker_fields() -> list[str]:
    return [
        ...
        "invalidRetryStopReasons", "retryableNonBoundFailures",
        "successScopeMismatches",
