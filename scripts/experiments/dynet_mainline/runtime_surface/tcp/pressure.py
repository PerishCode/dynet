...
def runtime_tcp_pressure_source(path: Path) -> dict[str, Any]:
    ...
    source["clean"] = runtime_tcp_pressure_clean(source)
    return source


def runtime_tcp_pressure_summary(sources: list[dict[str, Any]]) -> dict[str, Any]:
    return {
...
def runtime_tcp_pressure_clean(source: dict[str, Any]) -> bool:
    return (
        source["schema"] == TCP_PRESSURE_SCHEMA
