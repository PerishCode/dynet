...
def workload_probe_python(
    ...
    return (
        ...
        f"configured_concurrency_limit = {int(concurrency_limit or 0)!r}\n"
        ...
        "capture_ports = defaultdict(lambda: {'toTargetPackets': 0, 'fromTargetPackets': 0, 'synPackets': 0, 'synAckPackets': 0})\n"
        "capture_meta = {'enabled': any(str(item.get('probe') or 'https-head') != 'dns' for item in entries), 'started': False, 'rawLinesStored': False, 'rawPcapStored': False, 'interface': 'dynet0', 'filter': 'tcp and port 443'}\n"
        ...
        "def record_capture_port(port, field):\n"
        "    with capture_lock:\n"
        "        capture_ports[int(port)][field] += 1\n"
        "def parse_capture_line(line):\n"
        "    if ' IP ' in line:\n"
        "        line = line.split(' IP ', 1)[1]\n"
        "    elif line.startswith('IP '):\n"
        "        line = line[3:]\n"
        "    else:\n"
        "        return\n"
        "    endpoints = line.split(':', 1)[0]\n"
        "    if ' > ' not in endpoints:\n"
        "        return\n"
        "    left, right = endpoints.split(' > ', 1)\n"
        "    src_port = endpoint_port(left)\n"
        "    dst_port = endpoint_port(right)\n"
        "    flags = re.search(r'Flags \\[([^\\]]+)\\]', line)\n"
        "    flag_text = flags.group(1) if flags else ''\n"
        "    if dst_port == 443 and src_port is not None:\n"
        "        record_capture_port(src_port, 'toTargetPackets')\n"
        "        if 'S' in flag_text and '.' not in flag_text:\n"
        "            record_capture_port(src_port, 'synPackets')\n"
        "    elif src_port == 443 and dst_port is not None:\n"
        "        record_capture_port(dst_port, 'fromTargetPackets')\n"
        "        if 'S' in flag_text and '.' in flag_text:\n"
        "            record_capture_port(dst_port, 'synAckPackets')\n"
        "def capture_reader(proc):\n"
        "    try:\n"
        "        for line in proc.stdout or []:\n"
        "            parse_capture_line(line)\n"
        "    except Exception as exc:\n"
        "        capture_meta['readerErrorType'] = classify(exc)\n"
        "def start_tun_capture():\n"
        "    if not capture_meta['enabled']:\n"
        "        return None, None\n"
        "    try:\n"
        "        proc = subprocess.Popen(['sudo', 'tcpdump', '-i', 'dynet0', '-n', '-l', '-tt', 'tcp', 'and', 'port', '443'], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True)\n"
        "        thread = threading.Thread(target=capture_reader, args=(proc,), name='tun-capture')\n"
        "        thread.daemon = True\n"
        "        thread.start()\n"
        "        capture_meta['started'] = True\n"
        "        return proc, thread\n"
        "    except Exception as exc:\n"
        "        capture_meta['started'] = False\n"
        "        capture_meta['errorType'] = classify(exc)\n"
        "        return None, None\n"
        "def stop_tun_capture(proc, thread):\n"
        "    if proc is None:\n"
        "        return\n"
        "    try:\n"
        "        proc.terminate()\n"
        ...
        "def tun_capture_summary():\n"
        "    with capture_lock:\n"
        "        ports = [{'localPort': port, **counts} for port, counts in sorted(capture_ports.items())]\n"
        "    return {**capture_meta, 'ports': ports, 'portCount': len(ports)}\n"
        ...
        "    return {'_socket': sock, 'connectedFamily': 'ipv4', 'localPort': local_port, 'localAddressStored': False, 'peerMatchesSelectedAddress': sock.getpeername()[0] == ip}\n"
        ...
        "    limit = workload_concurrency_limit()\n"
        "    threads = limit if concurrent_workload else (1 if entries else 0)\n"
        "    return {'enabled': concurrent_workload, 'entryCount': len(entries), 'threadsStarted': threads, 'limit': limit, 'limited': concurrent_workload and limit < len(entries)}\n"
        "def workload_concurrency_limit():\n"
        "    if not concurrent_workload:\n"
        "        return 1 if entries else 0\n"
        "    if configured_concurrency_limit > 0:\n"
        "        return max(1, min(configured_concurrency_limit, len(entries)))\n"
        "    return len(entries)\n"
        "def schedule_wait(offset):\n"
        ...
        "def run_entry(index, entry):\n"
        "        offset = int(entry.get('scheduledOffsetMs') or 0)\n"
        "        lag = schedule_wait(offset)\n"
        "        item_start = time.monotonic()\n"
        "        item_started_at = unix_ms()\n"
        "        domain = str(entry.get('domain', '')).lower().strip('.')\n"
        "        probe = str(entry.get('probe') or 'https-head')\n"
        "        result = {'entryIndex': index, 'id': entry.get('id'), 'startedAtUnixMs': item_started_at, 'bucket': entry.get('bucket'), 'behavior': entry.get('behavior'), 'groupId': entry.get('groupId'), 'domain': domain, 'probe': probe, 'scheduledOffsetMs': entry.get('scheduledOffsetMs'), 'scheduleLagMs': lag, 'stages': []}\n"
        ...
        "                hold_after_connect_ms = int(entry.get('holdAfterConnectMs') or 0)\n"
        "                route_settle_ms = int(entry.get('routeSettleMs') or workload.get('routeSettleMs') or 0)\n"
        "                result.update(public(run_stage(result, 'route', lambda ip=ip: route_ip(ip))))\n"
        "                if route_settle_ms > 0:\n"
        "                    result['routeSettleMs'] = route_settle_ms\n"
        "                    time.sleep(route_settle_ms / 1000)\n"
        "                tcp = run_stage(result, 'tcp-connect', lambda ip=ip: tcp_connect(ip))\n"
        "                sock = tcp.get('_socket')\n"
        "                result.update(public(tcp))\n"
        "                if hold_after_connect_ms > 0:\n"
        "                    result['holdAfterConnectMs'] = hold_after_connect_ms\n"
        "                    time.sleep(hold_after_connect_ms / 1000)\n"
        ...
        "                        return\n"
        "                    next_index['value'] += 1\n"
        "                run_entry(index, entries[index])\n"
        "        for index in range(limit):\n"
        "            thread = threading.Thread(target=worker, name='workload-worker-%03d' % index)\n"
        "            thread.start()\n"
        ...
        "started = time.monotonic()\n"
        "results = []\n"
        "capture_proc, capture_thread = start_tun_capture()\n"
        "try:\n"
        "    run_entries()\n"
        "finally:\n"
        "    stop_tun_capture(capture_proc, capture_thread)\n"
        "    with routes_lock:\n"
        "        cleanup_routes = sorted(routes)\n"
        "    for ip in cleanup_routes:\n"
        "        subprocess.run(['sudo', 'ip', 'route', 'del', ip + '/32', 'dev', 'dynet0'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)\n"
        "results = sorted(results, key=lambda row: int(row.get('entryIndex') or 0))\n"
        "summary = {'schema': '" + WORKLOAD_PROBE_SCHEMA + "', 'manifestSchema': manifest.get('schema'), 'environment': manifest.get('environment'), 'seed': manifest.get('seed'), 'workload': workload, 'concurrency': concurrency_summary(), 'tunCapture': tun_capture_summary(), 'privacy': {'identityInformationSent': False, 'cookiesSent': False, 'authorizationSent': False, 'responseBodiesStored': False, 'responseHeadersStored': False, 'resolvedIpAddressesStored': False}, 'totals': aggregate(results), 'byBucket': grouped(results, 'bucket'), 'byBehavior': grouped(results, 'behavior'), 'byProbe': grouped(results, 'probe'), 'byStage': stage_groups(results), 'errors': top_errors(results), 'durationActualMs': elapsed_ms(started), 'results': results}\n"
