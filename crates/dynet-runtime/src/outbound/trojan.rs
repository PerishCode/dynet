...
use super::{connect_tcp_socket, TcpTarget};
...
pub(super) trait TrojanTransport: Read + Write {
    #[allow(dead_code)]
...
pub(super) fn connect_tcp(
    ...
    let tcp = connect_tcp_socket(&spec.server, spec.server_port, mark)?;
    ...
    let mut stream = StreamOwned::new(connection, Box::new(tcp) as Box<dyn TrojanTransport>);
    while stream.conn.is_handshaking() {
        stream
            .conn
            .complete_io(&mut stream.sock)
            .map_err(|error| format!("failed Trojan TLS handshake with `{}`: {error}", spec.sni))?;
    }
    stream
